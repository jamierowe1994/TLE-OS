...
export const FRONT: NavItem[] = [
  ...
  // questions — so Leads opens rather than just navigating.
  {
    ...
    icon: "target",
    children: [
      /* Always split, never "All leads" (James, 11 Sep 2026): add one, or
         go straight to a side. The parent opens this drawer rather than
         navigating, so the first child is the action. */
      { href: "/leads?new=1", label: "Add new lead" },
...
const WORKSPACES: Workspace[] = [
  ...
  {
    /* One door, landing on the dashboard (James, 5 Sep: the board is where
       a deal is worked, the dashboard is where she sees what needs working).
       Her screens are the rail inside, drawn by app/(os)/pre-tenancy/layout,
       not children here (James, 12 Sep: "when she clicks pre-tenancy, it'll
       launch her into an admin section"). */
