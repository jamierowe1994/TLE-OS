...
/**
 * The in-flight walk for a key, held as a PROMISE rather than just a flag.
 *
 * A Set only answered "is something running"; a caller who needed the answer
 * had no way to join it, so it got null and had to poll. Holding the promise
 * means a caller can WAIT on the exact same walk instead of starting another
 ...
 *
 * THE BUG THIS EXISTS FOR. A cold key returns null and computes behind, which
 * is right for a page that can show a spinner and poll. It is WRONG for
 * anything COMPOSING two cached values: the outer call got null for the inner
 * one, treated it as "nothing", and produced a confident answer full of
 * zeros — with no error anywhere, because nothing had failed. It had simply
 * not finished.
 *
 * So a page should keep the default and poll. A function that needs the value
 * to do its own arithmetic must pass `wait`, and take the time.
 */
async function cachedAsync<T>(
  rawKey: string,
  run: () => Promise<T>,
  opts: { wait?: boolean } = {}
): Promise<T | null> {
  const key = `${CACHE_VERSION}:${rawKey}`;
  return cachedAsyncInner(key, run, opts);
}
...
async function cachedAsyncInner<T>(
  key: string,
  run: () => Promise<T>,
  opts: { wait?: boolean } = {}
): Promise<T | null> {
  ...
  if (!running.has(key) && !cooling) {
    const walk = (async () => {
      ...
      if (data == null) {
        // A null RUN is a failure — "we couldn't find out" — not an answer.
      ...
      await writeCache(key, data).catch(() => {
  ...
  // for the same key shares this promise, so a cold page firing eight requests
  // costs one walk, not eight.
  if (opts.wait) {
    const inFlight = running.get(key) as Promise<T | null> | undefined;
    if (inFlight) return (await inFlight) ?? ((hit?.data as T) ?? null);
    // Nothing in flight and we were asked to wait — that means we are cooling
    // off a recent failure. Say so with the stale value or null, rather than
...
import { exVat } from "@/lib/business/format";
export { exVat };
...
async function computeIncomeRange(
   ...
   *
   * It used to poll — twenty attempts, three seconds apart, hoping. It now
   * JOINS the same walk via `{ wait: true }`, which is both faster and honest:
   * one walk, one answer, and no arbitrary sixty-second ceiling that could
   * expire mid-computation and hand back a book of nobody.
   *
   * We are already inside a multi-minute background walk, so waiting costs
   * nothing.
   */
  const book = await getPortfolioBook({ wait: true }).catch(() => null);
