...
| J23 | **Check all of Susan's figures** | **CLAUDE'S HALF DONE 14 Sep - the stored figures reconcile, and two real gaps found.** Read from production, read-only. `history_funnels` holds an unbroken run 2025-09 to 2026-08 and `gci_months` 2025-08 to 2026-08, starting exactly on `MONEY_FLOOR`; **the current month is correctly absent from both**, which is the design (the live month must never borrow a seed entry - that is how July's KPIs once appeared under an August pill). The overview's month handling is now off the clock in all five places that used to hardcode July. **Gap 1: nothing closes the month.** `/api/business/backfill` is the sweep that freezes a closed month, and no cron calls it - `os-cron-daily` runs eight routes at 06:00 and that is not one. A month only freezes if somebody happens to look at it. **Gap 2, and it is why Gap 1 would have bitten twice:** 18 cron-callable routes read `x-cron-key`, and the only two reading `x-cron-secret` were the two business-figures routes. Adding backfill to a cron the house way would have returned 401 on schedule, for ever, watched by nobody. Both spellings now accepted. **James: add backfill to `os-cron-daily`** (line below). PayProp UK's token is present, connected 5 Sep, scopes still empty - that is C9, not new. | Claude, then Susan | |
...
| F1.4 | ~~Viewing feedback is never recorded~~ **ANSWERED AND FIXED 14 Sep. It was ours, not theirs.** The team records feedback diligently - REX holds over 10,000 feedback records and the newest carry today's date. The OS could never show it: `VIEWING_OUTCOMES` in `lib/diary.ts` was a hardcoded map of ONE sample id, and a live viewing's id is `rex-<id>`, so "Feedback in" counted to zero on every real book whatever the team wrote, and every past viewing fell into "Feedback due". Feedback now comes out of REX's own Feedback service (`lib/rex-feedback.ts`) and travels on the appointment: **Feedback in went 0 to 4, Feedback due 29 to 26**, and opening a viewing shows the written answers, the Hot/Warm/Cold level and who recorded it. **The honest remaining number is a process one: only 40 of the 412 past viewings on the current rental book have any feedback in REX at all** - worth raising with Susan as a habit, not a bug. | Found 10 Sep, closed 14 Sep |
| F1.5 | ~~Landlord feedback report, per property, on Viewings~~ **DONE 14 Sep.** A third view on Viewings beside Calendar and Diary: every past viewing grouped by property, what was said, REX's Hot/Warm/Cold, and **Copy for the landlord** - which drops the viewers' names, because the landlord portal's own rule is "never a full name before an offer" and a report that leaked one would make that discretion pointless. Grouped by REX listing id, not by address text. It could not be built until the OS could see REX feedback at all (F1.4). The screen's caveat is replaced rather than deleted: **4 of the last 30 viewings have feedback written up in REX**, and the report names what is missing rather than hiding it. | Screen caveat, closed 14 Sep |
...
| F1.11 | ~~Tenant raises a repair from their portal~~ **DONE 15 Sep.** The tenant portal's Maintenance page was a paragraph and a mailto saying "reporting from this page comes with your move-in". It now takes a report and raises a works order with `reported_by: Tenant`, and the tenant sees what they have reported and where it has got to, in words with no money and no blame in them. **The property comes from the SESSION, never the body** - a propertyId in a request would let any tenant login raise work against somebody else's home, and the first anyone would know is a contractor knocking on the wrong door. Proved: no session refuses both verbs, an OS staff cookie does not open it, and a body carrying someone else's propertyId is refused and never read. **Every report lands ROUTINE and an agent re-grades it** - emergency means attend within 24 hours and the clock starts on report, so a form that let a tenant choose it would hand them the agency's own SLA clock. The page tells them to ring for anything dangerous. **James/Susan: confirm a tenant should never set the urgency themselves.** | Was: no tenant route exists |
| F1.12 | ~~Tenant sends documents from their portal~~ **DONE 15 Sep - J6 is closed.** Send us something on the tenant's Documents page: a kind (ID, right to rent, a reference, proof of income, proof of address), a PDF or a photograph up to 25MB, and a list of what they have sent. **And the other end, which is the half that makes it real** - what the tenants on a deal have sent shows on the pre-tenancy deal file. The join was checked, not assumed: the agent's file keys on `deal.app.id` and the tenant's view sets its id from exactly that, so it could not silently never match and sit saying "nothing yet" for ever. The R2 prefix comes from the SESSION, never a ref in the request, or one tenant could write into another's prefix and the reading route would hand it over quite correctly. Reads take the account id and document id TOGETHER rather than fetching by id and checking after. Proved: both tenant routes refuse anonymous AND an OS staff cookie; the agent route refuses anonymous and answers staff. |
...
| F2.9 | Inspections | **Printable report DONE 14 Sep** - Print the report on a written-up visit: the landlord's own document, grouped room by room, saying the same things the emailed version says and no more (two versions of one visit is an argument at a deposit hearing). It leaves out the tenant's access link, email and phone - the link is a bearer token to their own page and does not belong in somebody else's filing cabinet. **Works-order link DONE 14 Sep too** - Raise the works order sits on the finding, and the job opens on Maintenance carrying the room, the item and what was seen, `reported_by: Inspection` (already an option on a works order before this existed - the system expected it). The trade and the urgency are asked for, not guessed: "window catch does not hold shut" is a locksmith or a joiner depending on the window. Raising twice is refused. Proved end to end against the local database - works order 1015 from finding w1, and the finding now carries its id. **Photographs DONE 15 Sep - F2.9 is closed.** A finding takes photographs through the same /api/r2/upload every listing photo and compliance certificate goes through, so the server decides what may be stored rather than the browser, and they file under the visit's own prefix. They print on the report beside the finding they belong to. **One honest gap recorded on the screen rather than papered over: the EMAILED copy is still words only** - its template is text - so an agent sends the print when the pictures are the point. |
