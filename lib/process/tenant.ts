...
export const TENANT_PROCESS: ProcessMap = {
  ...
  nodes: [
    ...
    n({ id: "enquiry-reply", kind: "email", lane: "spine", col: 1, status: "planned", stage: "enquired", title: "About the home you asked about", blurb: "Straight away. Is it still available, what it costs to move in, the next viewing slots, and the passport. To write.", trigger: { on: "rex.lead.created", after: "straight away" } }),
    n({ id: "added-welcome", kind: "email", lane: "side", col: 1, row: -1, status: "planned", title: "Let's find you a home", blurb: "No property to talk about, so it leads on the search: what we need to know, what we have on, and the passport. To write.", trigger: { on: "contact.added", after: "straight away" } }),
    ...
    n({ id: "nudge-1", kind: "email", lane: "nurture", col: 4, row: -2, status: "planned", title: "Nudge: two days", blurb: "A short reminder with the same link. To write.", trigger: { on: "passport.not-started", after: "2 days" } }),
    n({ id: "nudge-2", kind: "email", lane: "nurture", col: 5, row: -2, status: "planned", title: "Nudge: a week", blurb: "Why the passport matters: one form for every property. To write.", trigger: { on: "passport.not-started", after: "7 days" } }),

    /* ── The hunt ────────────────────────────────────────────────────── */
    n({ id: "matches", kind: "email", lane: "spine", col: 3, status: "planned", stage: "matched", title: "Homes that fit", blurb: "The properties that match what they told us, the same day they told us. The picker exists on the lead (Email properties); sending is off until the outbound switch is on. Copy to write.", trigger: { on: "tenant.qualified", after: "same day" } }),
    ...
    n({ id: "matches-again", kind: "email", lane: "nurture", col: 6, row: 1, status: "planned", stage: "matched", title: "Anything close?", blurb: "Have the criteria changed, and here is what has come on since. To write.", trigger: { on: "matches.no-reply", after: "4 days" } }),
    ...
    n({ id: "viewing-confirmed", kind: "email", lane: "spine", col: 6, status: "planned", stage: "viewing", title: "Viewing confirmed", blurb: "Straight away: the address, the time, who is meeting them, how to get in - and how to move or cancel it without ringing. To write, and the cancel link needs building.", trigger: { on: "viewing.booked", after: "straight away" } }),
    n({ id: "viewing-reminder", kind: "email", lane: "spine", col: 7, status: "planned", stage: "viewing", title: "Your viewing is today", blurb: "7am on the day: the time, the address on a map, the agent's mobile. The one email that stops a no-show. To write, and it needs a cron of its own.", trigger: { on: "viewing.day", after: "7am on the day" } }),
    ...
    n({ id: "rebook", kind: "email", lane: "nurture", col: 9, row: 1, status: "planned", stage: "viewing", title: "Shall we rebook?", blurb: "Two lines and three new slots. To write.", trigger: { on: "viewing.no-show", after: "2 hours" } }),

    n({ id: "feedback", kind: "email", lane: "spine", col: 9, status: "planned", stage: "viewed", title: "How was it?", blurb: "Two hours after, while they still remember it. One tap: loved it, not for me, or in between. The answer goes to the landlord and back into the matching. To write.", trigger: { on: "viewing.done", after: "2 hours" } }),
    ...
    n({ id: "not-for-them", kind: "email", lane: "nurture", col: 10, row: 1, status: "planned", stage: "matched", title: "Not that one - try these", blurb: "Same day. What they said they didn't like, and three that don't have it. To write.", trigger: { on: "viewing.feedback", after: "same day" } }),
    ...
    n({ id: "apply-received", kind: "email", lane: "spine", col: 12, status: "planned", stage: "offer", title: "We have your application", blurb: "What we do with it, when they will hear, and what to have ready if it is accepted. To write.", trigger: { on: "application.received", after: "straight away" } }),
    ...
    n({ id: "declined", kind: "email", lane: "nurture", col: 13, row: 1, status: "planned", stage: "declined", title: "Not this one - and here is what is next", blurb: "Same day, never silence. Why, if we know it. Then straight back to matching with a fresh set attached - a declined offer is the start of the search again, not the end of it. To write.", trigger: { on: "application.declined", after: "same day" } }),
    ...
    n({ id: "accepted-email", kind: "email", lane: "spine", col: 15, status: "planned", stage: "deal_started", title: "It's yours", blurb: "The holding fee, what it does and what happens to it, and the week ahead in order. To write.", trigger: { on: "propoly.deal.started", after: "straight away" } }),
    ...
    /* ── Referencing ─────────────────────────────────────────────────── */
    n({ id: "referencing-invite", kind: "email", lane: "spine", col: 17, status: "planned", stage: "referencing", title: "Time to get referenced", blurb: "What is asked, why, how long it takes, and what to have to hand - payslips, the last landlord's email, dates. To write.", trigger: { on: "propoly.deal.referencing", after: "straight away" } }),
    ...
    n({ id: "guarantor-invite", kind: "email", lane: "side", col: 18, row: -1, status: "planned", stage: "referencing", title: "Your guarantor", blurb: "When one is needed: what they are agreeing to, and their own link. To write.", trigger: { on: "referencing.guarantor.needed" } }),
    ...
    n({ id: "referencing-chase", kind: "email", lane: "nurture", col: 19, row: 1, status: "planned", stage: "referencing", title: "Chase: your references", blurb: "What is still missing, by name, and how long it will take them. To write.", trigger: { on: "referencing.stalled", after: "3 days" } }),
