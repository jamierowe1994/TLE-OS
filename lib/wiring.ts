...
export const WIRING: WiringRow[] = [
  ...
  },
  {
    ...
    state: "untested",
    note: "EDITING both is now live and proven: the write-up since 29 Aug, and on 15 Sep the Marketing tab began saving rent, deposit, dates, key features, photo order and the rooms and services — the property half through Properties/update, which is on the allowlist. CREATING one from scratch is still untested and + Add new listing is still not connected, which is the next job on this row.",
  },
  {
    system: "rex",
    area: "Listings & publishing",
    ...
    state: "proven",
    note: "The save carries the editing person's own REX token, falling back to the office service account when there isn't one — four people have linked their REX account so far. Proven 15 Sep: James's save stamped the PROPERTY as James Rowe, and the publish before it recorded him as the publisher. The LISTING shows 'System User' a second or two later, because REX's own portal-upload job re-saves the listing after any change and takes the name with it; that is REX's stamp, not our fallback. The OS keeps its own log of who changed what either way.",
  },
  {
    system: "rex",
    area: "Listings & publishing",
    ...
    state: "untested",
    note: "BUILT 15 Sep 2026 and not yet run for real: Add photos and Add floor plan on the Marketing tab put the file in our own storage, then hand REX a signed link with Upload/uploadFileFromUrl and add the returned file to the listing's images or floor plans. Both calls are on the allowlist, so the only thing left is one supervised upload. REX takes 50 photos and 5 floor plans on a listing.",
  },
  {
    system: "rex",
    area: "Listings & publishing",
