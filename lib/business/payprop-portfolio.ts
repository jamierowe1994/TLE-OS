...
/**
 * The walk in progress, held as a PROMISE not a boolean.
 *
 * `running = true` only answered "is something happening"; a caller who needed
 * the book had no way to join it. So the one place that genuinely needs the
 * value — per-agent attribution inside the income walk — polled this function
 * every 3 seconds, twenty times, hoping. Holding the promise lets it simply
 * wait for the same walk.
 */
let inFlight: Promise<PortfolioBook | null> | null = null;

/**
 * The managed book, grouped by responsible agent.
 *
 * Non-blocking by default, in the same way as the income figures: serves
 * what's cached and refreshes behind. Pass `{ wait: true }` when you need the
 * ANSWER rather than a fast reply — composing this into arithmetic without
 * waiting is how a cold key becomes "zero agents" with no error anywhere.
 */
export async function getPortfolioBook(
  ...
  if (!inFlight && Date.now() - failedAt > FAILURE_COOLDOWN_MS) {
    inFlight = (async () => {
  ...
  // Join the walk already running rather than starting another or guessing.
  if (opts.wait && inFlight) return (await inFlight) ?? cache?.data ?? null;
