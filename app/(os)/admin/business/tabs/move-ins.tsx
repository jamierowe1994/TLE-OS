...
export default function MoveInsTab({ month, seed }: { month: string; seed: SeedData }) {
  ...
  useEffect(() => {
    let off = false;
    setRowsLoading(true);
    fetch(`/api/business/move-in-rows?month=${encodeURIComponent(tableMonth)}`, {
      cache: "no-store",
    })
  ...
  useEffect(() => {
    let cancelled = false;
    fetch("/api/business/move-in-tracker", { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => {
  ...
  const loadOverrides = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/business/actuals?month=${encodeURIComponent(month)}`,
        { cache: "no-store" }
      );
      if (!res.ok) return;
