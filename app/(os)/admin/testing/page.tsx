...
import PhasesPanel from "@/components/testing/PhasesPanel";
import {
  JOURNEYS,
  KITS,
  LIGHT_WORDS,
...
export default function TestingPage() {
  ...
  const [scope, setScope] = useState<"core" | "all" | "files" | "phases">("core");
        ...
        <div role="radiogroup" aria-label="Which tests" className="flex overflow-hidden rounded-lg border border-line/80">
          {(
            [
              ...
              ["all", "Everything"],
              ["files", "Test files"],
              /* The pilot's three phases (lib/phases, 21 Sep 2026). Last, not
                 first: it is pressed three times ever, the others daily. */
              ["phases", "Phases"],
            ] as const
          ).map(([id, label]) => (
            <button
