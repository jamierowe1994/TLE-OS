...
| 5 | Compliance tracker + 30/14/7 reminders | Michael | **built** (22 Aug), rescoped 10 Sep to the 404 homes the agency is answerable for — managed in REX PM and not let only, which is the breakdown sheet's own scope. The jobs this left for a person are queued in `docs/MICHAEL-COMPLIANCE-VIEW.md`. Tracker live off REX; the two chase emails send behind the Certificate chases switch and go to agent and landlord both |
...
| 16 | REX PM integration | — | **re-opened 6 Sep**: REX PM is a separate product (Alfie, alfie.app.rexsoftware.com) with its own 724 properties and 18,244 compliance entries, not the empty PM tables inside REX CRM probed on 22 Aug. No API key; pulled through James's signed-in session (see the launch-week to-do). Its properties are not linked to REX CRM's, so the address matcher joins them |
...
| James: certificates into REX. The approved PLC pack now writes its gas, EICR, EPC and licence into REX's compliance table (5 Sep) - the table the tracker, the chases and REX PM read. It needs the Certificates into REX switch on Admin and `REX_ALLOW_WRITES` on TLE-OS naming `ComplianceEntries/create` and `Upload/uploadFileFromUrl`. **Proven live 5 Sep on 10 Cherry Road**: EICR entry 1024249 with its file, gas entry 1024248 (file missing - written before the file field was fixed; REX's update will not attach one afterwards, so only a fresh entry carries a file). The tracker cleared the EICR within a minute. **The backlog went in overnight 5-6 Sep**: every Propoly document pulled to James's Desktop (7,640 files, 618 properties), each folder matched to its REX property by one resolver (477 folders confident, 5 to check, 53 with nothing in REX), each gas, EICR and EPC read for its dates and address, and 1,326 certificates written into REX with the file and filed in the listing's Documents. Held back, never written: 44 certificates that name a different property, 33 with no readable expiry, 147 files in 58 folders REX does not hold. What the run showed: the tracker only fell 910 → 845 because 101 of the 251 tracked properties are not in Propoly at all and 28 have no documents there, and 562 of the outstanding rows are types Propoly never held (legionella, alarms, fire, PAT, licence) | the 911 is now a true figure: what is left is missing from Propoly too, and needs Kirstie's PLC intake. **Decided 6 Sep (James)**: gas, EICR and EPC on every home; alarms, fire risk, PAT and legionella tracked on HMOs only - one rule in lib/compliance, read by the tracker, the portal, the drawers and Steve. **The property file (6 Sep, morning)**: one panel keyed on the REX property, the same on the listing's Compliance tab, the application, the market appraisal's page and Portfolio - every duty and where it stands in REX, the files on hand, and Attach (reads the document, then files it here and in REX). An appraisal with no property holds the file against the address; the matcher moves it onto the property when it is linked. PAT and legionella went in the same night (67 and 56). **REX PM (Alfie) pull, 6 Sep morning**: 2,520 live certificates found in REX PM's own store, 627 filed on 323 REX properties using REX PM's dates (no reading needed); gas in date 84 → 171, EICR 301 → 331, EPC 423 → 433 across the 451 managed homes. Vault now 1,963 certificates on 529 properties. Before-and-after page sent to James for Susan. **Then the gas gap explained (6 Sep, midday)**: the 451 mixes 175 managed with 129 let-only and 147 with no service set; scoped to managed, gas needed attention on 71 of 175. The landlord's signed terms of business carry a "NO Gas at property" box: 128 terms read, 59 homes say no gas, 52 gas entries created in REX as not required citing the terms (7 already were). Susan's breakdown page: every home, every certificate, scope by service, what REX PM says, where each file came from. **Afternoon, from James's dig into 14 Martins Lane and 2 Norwich Street**: REX PM carries a property compliance category ("No Gas", "HMO Standard", …) - read for all 724, 58 more homes marked not required in REX (108 in all); and a certificate on ONE room of a shared house now lands on the house and every room (matcher, both the runner and lib/property-match) - 79 more room certificates filed. Managed gas needing attention: 71 → 22 of 124 with a gas duty. **Late afternoon: the OS's own property record** (`os_properties`, lib/os-properties.ts): one row per REX PM home (724; 522 active), linked to its REX property by address (394) or held as "Not on REX" (128), carrying REX PM's categories as facts (53 HMOs, 174 no gas). The managed book and the compliance book append the Not on REX homes, so Portfolio reads 579 and the tracker counts them; their 471 live certificates were pulled from REX PM into the OS vault (no REX write). Portfolio has a Not on REX pill and filter; Susan's sheet a Not on REX scope. REX's service field on matched homes untouched - Susan's call. **End of day**: the OS represents all 522 REX PM-managed homes (551 records, since REX keeps some houses as several rooms): 423 under a REX property, 128 Not on REX. On that scope: gas in date 314, expired 68, no record 23, no gas 146; EICR in date 498, expired 25, none 28; EPC in date 531, expired 9, none 11 |
