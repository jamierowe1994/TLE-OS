 ...
 * to name what is short. Handing over is the easy half; Howard's flow already
 * does it.
 */
...
export default function HandoffPanel({ applicationId }: { applicationId: string }) {
  ...
  async function send() {
    ...
    try {
      ...
      setResult(d.ok ? "Handed over." : (d.error ?? "That didn't go through."));
    } catch (e) {
  ...
  return (
    <div className="rounded-2xl border border-line/80 bg-panel p-5">
      ...
      <button
        type="button"
        onClick={send}
        disabled={!ready || sending || !h.flowConfigured}
      ...
      {!h.flowConfigured && (
        <p className="mt-2 text-[11px] leading-relaxed text-muted">
          Howard&apos;s <span className="font-semibold">TLE: Application Accepted</span> flow
          does the Propoly creation, the listing update and both emails. It has a manual HTTP
          trigger, so the OS can call it rather than rebuild it — but the trigger URL carries a
          SAS signature and only appears on the flow&apos;s own trigger card in Power Automate.
          Set it as <span className="font-semibold">POWER_AUTOMATE_ACCEPTED_URL</span>.
        </p>
      )}
