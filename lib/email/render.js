...
export const BLOCK_TYPES = [
  ...
  { type: 'quote', label: 'Testimonial', hint: 'Somebody\u2019s words, in quotes, with a name under them' },
...
/* ── THE BRAND FACES, AND AN HONEST WARNING ─────────────────────────────────
   Everything below `web: true` is a Google font. Email clients are NOT
   browsers: Gmail (web and Android), Outlook on Windows and Yahoo all strip
   @font-face, so those recipients see the FALLBACK, not the brand face.
   Apple Mail, iOS Mail and Outlook.com do load them.

   That is why every web entry carries a deliberate fallback rather than a
   bare `cursive`: the fallback is what most of the list will actually read,
   so it has to look intentional on its own. Sacramento falls back to the
   system script faces, which is the closest any mail client gets.

   The builder labels these "not everywhere" so nobody picks one for body
   copy by accident. Use them for a display word; never for a paragraph. */
export const FONT_STACKS = [
  /* The type since 11 Sep 2026: Inter for body (first, so it is also the
     default for any block that names no font) and Manrope for headings.
     The older faces below stay so templates saved with them still render. */
  { key: 'inter',      label: 'Inter (brand body)', web: true,
    stack: "Inter, 'Helvetica Neue', Helvetica, Arial, sans-serif" },
  { key: 'manrope',    label: 'Manrope (brand headings)', web: true,
    stack: "Manrope, Inter, 'Helvetica Neue', Helvetica, Arial, sans-serif" },
  { key: 'montserrat', label: 'Montserrat (brand)', web: true,
    stack: "Montserrat, 'Helvetica Neue', Arial, sans-serif" },
  { key: 'lora',       label: 'Lora (brand)', web: true,
    stack: "Lora, Georgia, 'Times New Roman', serif" },
  /* Shantell Sans IS the OS's handwriting - globals.css sets --font-heading
     to it, so this is the face on "Admin", "Leads", every section title in
     the app. Using it in email is what makes an email look like it came from
     the same product. */
  { key: 'shantell',   label: 'Shantell Sans (the OS hand)', web: true,
    stack: "'Shantell Sans', 'Comic Sans MS', 'Segoe Print', cursive" },
  /* The signature script, matching --font-script in app/layout.tsx. NOTE the
     older note in the brand memory says Sacramento; the code loads Ms Madi,
     and the code is what ships. */
  { key: 'msmadi',     label: 'Ms Madi (signature script)', web: true,
    stack: "'Ms Madi', 'Snell Roundhand', 'Apple Chancery', cursive" },
...
/** The TLE palette, straight from "TLE Branding 3". Swatches, not a theme:
 *  a marketer picking a colour should be picking one of THESE. */
export const BRAND_COLOURS = [
  { key: 'red',   label: 'Expert Red',       hex: '#E31F36' },
  { key: 'grey',  label: 'Expert Grey',      hex: '#3B3B3C' },
  { key: 'white', label: 'Anti Flash White', hex: '#F1F1F1' },
  { key: 'clay',  label: 'Warm Clay',        hex: '#DE968F' },
  { key: 'mist',  label: 'Mist',             hex: '#FFE4DF' },
  { key: 'ink',   label: 'Near black',       hex: '#1C1D22' },
];

/** Google Fonts link for the web faces, so the clients that DO honour web
 *  fonts get them. Stripped harmlessly everywhere else. */
const WEBFONT_LINK =
  '<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400..700&family=Manrope:wght@500..800&family=Lora:ital,wght@0,400..700;1,400..700&family=Montserrat:wght@300..800&family=Ms+Madi&family=Shantell+Sans:ital,wght@0,300..800;1,300..800&display=swap" rel="stylesheet" />';
...
export function makeBlock(type) {
  ...
  switch (type) {
    ...
    case 'quote':
      return { type, id, text: 'They made the whole thing feel easy.', who: 'A landlord in Chorlton', align: 'left', color: '' };
    case 'divider':
...
function renderSocial(block, brand) {
  ...
  // A brand that brings its own icon PICTURES gets a plain row of them, no
  // chips. Pictures because Gmail and Outlook drop inline SVG, which left the
  // chip as an empty circle. Order is the brand's own.
  if (brand.socialImages && typeof brand.socialImages === 'object') {
    const row = Object.entries(brand.socialImages)
      .filter(([k, src]) => src && brand[k] && show[k] !== false)
      .map(([k, src]) => `<a href="${escapeHtml(brand[k])}" style="display:inline-block;text-decoration:none;margin:0 9px;"><img src="${escapeHtml(src)}" width="24" height="24" alt="${escapeHtml(k.charAt(0).toUpperCase() + k.slice(1))}" style="display:block;width:24px;height:24px;border:0;outline:none;" /></a>`)
      .join('');
    return row ? `<div style="text-align:center;line-height:0;padding:0 0 14px;">${row}</div>` : '';
  }
  const align = ['left', 'center', 'right'].includes(block.align) ? block.align : 'center';
...
function renderFooter(block, brand) {
  ...
  const unsub = block.unsubscribe !== false
    ...
    : '';
  // The comment is where the sender's own email footer goes when the mail
  // leaves their mailbox (lib/email-footer, 21 Sep 2026): under the words and
  // the sign-off, above the small print. An empty comment anywhere else.
  return `
    <!--os-agent-footer-->
...
function renderQuote(block, brand, ctx) {
  const align = ['left', 'center', 'right'].includes(block.align) ? block.align : 'left';
  const size = sizeOr(block.size, 19);
  const colour = escapeHtml(block.color || brand.bodyColor || '#1F2937');
  const accent = escapeHtml(brand.accentColor || '#e31f36');
  const words = escapeHtml(renderTokens(String(block.text || ''), ctx));
  const who = escapeHtml(renderTokens(String(block.who || ''), ctx));
  /* A table, not a blockquote: Outlook ignores margin on block elements, so a
     hand-rolled indent is the only one that survives it. */
  return `<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="${padStyleResolved(block)}">
  <tr>
    <td width="6" style="background:${accent};border-radius:3px;">&nbsp;</td>
    <td width="14">&nbsp;</td>
    <td style="text-align:${align};">
      <div style="margin:0;font-family:${fontStack(block.font || brand.bodyFont)};font-size:${size}px;line-height:1.5;font-style:italic;color:${colour};">&ldquo;${words}&rdquo;</div>
      ${who ? `<div style="margin:8px 0 0 0;font-family:${fontStack(brand.bodyFont)};font-size:13px;line-height:1.4;color:#6B7280;">${who}</div>` : ''}
    </td>
  </tr>
</table>`;
}
...
export function renderBlock(block, brand, ctx) {
  switch (block.type) {
    ...
    case 'quote': return renderQuote(block, brand, ctx);
