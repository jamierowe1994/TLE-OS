...
export default function HomeView({ v, welcome, base, q = "" }: { v: TenantHome; welcome: boolean; base: string; q?: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
          ...
          <div className="min-w-0 flex-1 py-1">
            {d ? (
              <>
                ...
                <Link href={to("/tenancy")} className="mt-5 inline-flex items-center gap-2 rounded-full border border-line/80 px-4 py-2 text-[13px] font-semibold transition-colors hover:border-ink">
        ...
        <div className="relative overflow-hidden rounded-[22px] bg-accent-soft p-6 sm:p-7" data-search style={rise(2)}>
          ...
          {v.next.href.startsWith("http") ? (
            ...
            <Link href={v.next.href.startsWith("/tenant/") ? to(v.next.href.slice("/tenant".length)) : v.next.href} className="relative mt-6 inline-flex items-center gap-2 rounded-full bg-accent-dark px-6 py-3 text-[14px] font-semibold text-white">
              {v.next.cta} <DoodleIcon name="trend-up" size={14} className="invert" />
            </Link>
          )}
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Tile icon="wallet" title="Rent & payments" href={to("/payments")} action="View all" i={4} rise={rise}>
          {d && money(d.rentPcm) ? (
            <>
        ...
        <Tile icon="setting" title="Maintenance" href={to("/maintenance")} action={d ? "New request" : null} i={6} rise={rise}>
          <div className="text-center">
            ...
            <Link href={to("/maintenance")} className="mt-4 inline-flex items-center gap-2 rounded-full border border-line/80 px-4 py-2 text-[12.5px] font-semibold transition-colors hover:border-ink">
