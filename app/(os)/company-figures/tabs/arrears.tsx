...
export default function ArrearsTab({ month, seed }: { month: string; seed: SeedData }) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/business/rent-collection?month=${encodeURIComponent(month)}`, { cache: "no-store" })
  ...
  const loadLog = useCallback(() => {
    fetch(`/api/business/arrears-history?month=${encodeURIComponent(month)}`, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
  ...
  useEffect(() => {
    ...
    setCaptured(true);
    fetch("/api/business/arrears-history", {
      method: "POST",
...
function ArrearsImport({
  ...
  const post = async (commit: boolean) => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/business/arrears-history", {
        method: "POST",
        headers: { "content-type": "application/json" },
