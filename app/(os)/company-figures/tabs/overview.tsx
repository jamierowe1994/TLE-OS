...
import { exVat, formatGBPCompact, formatGBPExact, formatNum, monthLabel } from "@/lib/business/format";
import { HISTORY_FLOOR, liveMonth, withinHistory } from "@/lib/business/roster";
...
export default function Overview({ month }: { month: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/business/overview?month=${sel}`, { cache: "no-store" });
    ...
    const run = async (refresh = false) => {
      ...
      try {
        const res = await fetch(
          `/api/business/live-business?month=${sel}${refresh ? "&refresh=1" : ""}`,
          { cache: "no-store" }
        );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/business/history-funnel", { cache: "no-store" });
        if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const res = await fetch("/api/business/ramp", { cache: "no-store" });
        if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    let tries = 0;
    const ask = () => {
      fetch(`/api/business/payprop-live?month=${sel}`, { cache: "no-store" })
        .then((r) => r.json())
        .then((res: {
