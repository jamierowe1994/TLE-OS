...
export const TENANT_PROCESS: ProcessMap = {
  ...
  nodes: [
    ...
    n({ id: "enquiry-reply", kind: "email", lane: "spine", col: 1, status: "reworked", emailId: "tenant-enquiry-reply", stage: "enquired", title: "About the home you asked about", blurb: "Within five minutes of the enquiry landing: is it still available, the rent, what it costs to move in, arrange the viewing by reply, and the passport. Sends once the Automatic tenant emails switch is on.", trigger: { on: "rex.lead.created", after: "straight away" } }),
    n({ id: "added-welcome", kind: "email", lane: "side", col: 1, row: -1, status: "reworked", emailId: "tenant-added-welcome", title: "Let's find you a home", blurb: "When an agent adds a tenant with no property: what we need to know, the passport to tell us in, and a promise to send what fits. Sends once the Automatic tenant emails switch is on.", trigger: { on: "contact.added", after: "straight away" } }),
    ...
    n({ id: "not-started", kind: "decision", lane: "nurture", col: 3, row: -2, status: "reworked", title: "Passport not started?", blurb: "Two and seven days after the invite with nothing typed. Checked hourly by lib/tenant-reminders.", trigger: { on: "passport.invited", after: "2 days" } }),
    n({ id: "nudge-1", kind: "email", lane: "nurture", col: 4, row: -2, status: "reworked", emailId: "tenant-passport-nudge-1", title: "Nudge: two days", blurb: "A short reminder with the same link. Sends hourly once the Tenant reminders switch is on.", trigger: { on: "passport.not-started", after: "2 days" } }),
    n({ id: "nudge-2", kind: "email", lane: "nurture", col: 5, row: -2, status: "reworked", emailId: "tenant-passport-nudge-2", title: "Nudge: a week", blurb: "Why the passport matters: one form for every property. The last reminder. Sends once the Tenant reminders switch is on.", trigger: { on: "passport.not-started", after: "7 days" } }),

    /* ── The hunt ────────────────────────────────────────────────────── */
    n({ id: "matches", kind: "email", lane: "spine", col: 3, status: "reworked", emailId: "tenant-matches", stage: "matched", title: "Homes that fit", blurb: "The homes the agent ticks on the lead (Email properties), sent as the agent with a button to their passport. Choosing is by reply. Sends once customer email is on.", trigger: { on: "tenant.qualified", after: "same day" } }),
    ...
    n({ id: "no-reply", kind: "decision", lane: "nurture", col: 5, row: 1, status: "reworked", stage: "matched", title: "Nothing back?", blurb: "Four days after the homes went, no viewing booked since. Checked hourly; only followed up when something new has come on near them.", trigger: { on: "matches.sent", after: "4 days" } }),
    n({ id: "matches-again", kind: "email", lane: "nurture", col: 6, row: 1, status: "reworked", emailId: "tenant-matches-again", stage: "matched", title: "Anything close?", blurb: "Have the criteria changed, and what has come on since near the homes we sent. Sends once the Automatic tenant emails switch is on.", trigger: { on: "matches.no-reply", after: "4 days" } }),
    ...
    n({ id: "viewing-reminder", kind: "email", lane: "spine", col: 7, status: "reworked", emailId: "viewing-reminder", stage: "viewing", title: "Your viewing is today", blurb: "From 7am on the day: the time, the address on a map, who is meeting them, and the agent's mobile where we have it. The one email that stops a no-show. Sends hourly once the Tenant reminders switch is on.", trigger: { on: "viewing.day", after: "7am on the day" } }),
    ...
    n({ id: "no-show", kind: "decision", lane: "nurture", col: 8, row: 1, status: "reworked", stage: "viewing", title: "Didn't turn up?", blurb: "The agent records it on the viewing drawer. Nobody wastes a slot twice, and nobody is written off for one missed morning.", trigger: { on: "viewing.no-show" } }),
    n({ id: "rebook", kind: "email", lane: "nurture", col: 9, row: 1, status: "reworked", emailId: "viewing-rebook", stage: "viewing", title: "Shall we rebook?", blurb: "Two hours after a no-show: we missed you, reply with a time. Sends once the Automatic tenant emails switch is on.", trigger: { on: "viewing.no-show", after: "2 hours" } }),

    n({ id: "feedback", kind: "email", lane: "spine", col: 9, status: "reworked", emailId: "viewing-feedback", stage: "viewed", title: "How was it?", blurb: "Two hours after the viewing ends, a link to their own feedback page on the home they saw. The answers and any offer go to the agent. Sends once the Automatic tenant emails switch is on.", trigger: { on: "viewing.done", after: "2 hours" } }),
    n({ id: "interested", kind: "decision", lane: "spine", col: 10, status: "reworked", stage: "viewed", title: "Do they want it?", blurb: "Their answer on the feedback page. Yes puts an offer to the agent (never above the asking rent). No sends homes nearby at a similar rent.", trigger: { on: "viewing.feedback" } }),
    n({ id: "not-for-them", kind: "email", lane: "nurture", col: 10, row: 1, status: "reworked", emailId: "viewing-not-for-them", stage: "matched", title: "Not that one - try these", blurb: "Straight after a no: what they said back to them, and live homes nearby at a similar rent. Sends once the Automatic tenant emails switch is on.", trigger: { on: "viewing.feedback", after: "same day" } }),
    ...
    n({ id: "apply-received", kind: "email", lane: "spine", col: 12, status: "reworked", emailId: "application-received", stage: "offer", title: "We have your application", blurb: "When REX shows a new application: what we do with it, when they will hear, the holding fee if it is a yes and what to have ready. Sends once the Automatic tenant emails switch is on.", trigger: { on: "application.received", after: "straight away" } }),
    n({ id: "landlord-answer", kind: "decision", lane: "spine", col: 13, status: "reworked", stage: "offer", title: "Accepted or declined?", blurb: "The landlord's answer, as REX's application status: accepted runs the handover, unsuccessful sends Not This One.", trigger: { on: "application.decided" } }),

    n({ id: "declined", kind: "email", lane: "nurture", col: 13, row: 1, status: "reworked", emailId: "application-declined", stage: "declined", title: "Not this one - and here is what is next", blurb: "Same day, never silence: sorry once, not the end, and live homes nearby. Sends once the Automatic tenant emails switch is on.", trigger: { on: "application.declined", after: "same day" } }),
    ...
    n({ id: "accepted-email", kind: "email", lane: "spine", col: 15, status: "reworked", emailId: "application-its-yours", stage: "deal_started", title: "It's yours", blurb: "The yes, the holding fee (what it is, where it goes, when it comes back) and every step to the keys. Sent by the offer-accepted handover in place of Howard's wording; shadow until the handover switch is on.", trigger: { on: "propoly.deal.started", after: "straight away" } }),
